...
Roland-Plan 是一个防遗忘型个人日程 / 任务管理 Web App。它帮助用户记录、查看和回顾需要完成的事情，也可以把大型计划拆成带时间范围的项目任务。

当前版本：`Roland-Plan v0.9.0 内测版`
...
普通一次性任务只保存在同一个 `tasks` 数组中。项目页不会创建另一套项目任务数组，只会为普通任务补充可选的项目归属、父任务和计划日期。

今日、本周、下周、项目、日历、搜索、回顾等页面都只是 `tasks` 的不同视图，按日期、项目、标签、完成状态等条件动态计算。
